...
export default async function SetPassword() {
  const u = await getUser('OFFICE');
  if (!u) redirect(LOGIN_PATH.OFFICE);
  if (!u.mustChangePassword) redirect('/office');
  const { t } = await getT();
  return (
    <div className="flex min-h-dvh items-center justify-center bg-gradient-to-br from-navy-800 to-navy-700 p-4">
      <div className="w-full max-w-md">
        ...
        <div className="card p-6">
          ...
          <ForcedPasswordForm portal="OFFICE" username={u.username} />
        </div>
      </div>
    </div>
  );
