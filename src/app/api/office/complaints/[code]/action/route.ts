...
import { requireApiUser, has, type AuthUser } from '@/lib/auth';
...
export const POST = route<Ctx>(async (req, { params }) => {
  const u = await requireApiUser('OFFICE'); // complaint actions only from the officer portal
  ...
  switch (action) {
    ...
    case 'schedule_inspection': {
      ...
      const inspector = await assignableUser(u, inspectorId, true);
      ...
      await audit(u, { action: 'complaint.inspection_scheduled', entityType: 'complaint', entityId: code, targetUserId: inspectorId, newValue: { inspector: inspector.full_name, due } });
      break;
    }
    ...
    case 'inspect': {
      ...
      if (u.role === 'FIELD_STAFF' && c.inspector_id !== u.id) throw forbidden('Only the assigned inspector can record this inspection');
      ...
      await audit(u, { action: 'complaint.inspected', entityType: 'complaint', entityId: code, newValue: { outcome, notes, lat: geo.latitude, lng: geo.longitude } });
      if (outcome === 'VERIFIED') {
    ...
    case 'reassign': {
      ...
      const assignee = await assignableUser(u, assigneeId, false);
      ...
      const prev = await sql`UPDATE assignments SET status = 'REASSIGNED' WHERE complaint_id = ${id} AND purpose = 'WORK' AND status IN ('PENDING','ACCEPTED','IN_PROGRESS') RETURNING assigned_to`;
      await sql`INSERT INTO assignments (complaint_id, assigned_to, assigned_by, department_id, purpose, priority, due_at, note)
                VALUES (${id}, ${assigneeId}, ${u.id}, ${assignee.department_id ?? c.department_id}, 'WORK', ${priority}, ${due}, ${str(data.note)})`;
      await transition(id, 'ASSIGNED', u, {
        note: `${reassign ? 'Reassigned' : 'Assigned'} to ${assignee.full_name}${data.note ? ` — ${str(data.note)}` : ''}`,
        set: { assigned_to: assigneeId, priority, department_id: assignee.department_id ?? c.department_id },
        skipCitizenNotify: reassign,
      });
      await notify(assigneeId, 'STAFF_ASSIGNED', { code, category_en: c.cat_en as string, category_ta: c.cat_ta as string }, id);
      await audit(u, { action: reassign ? 'complaint.reassigned' : 'complaint.assigned', entityType: 'complaint', entityId: code, targetUserId: assigneeId,
        oldValue: { assigned_to: prev.map((p) => p.assigned_to) }, newValue: { assigned_to: assigneeId, priority, due } });
      break;
    }
    ...
    case 'complete': {
      ...
      const [a] = await sql`SELECT * FROM assignments WHERE complaint_id = ${id} AND purpose = 'WORK' AND assigned_to = ${u.id}
                            AND status IN ('PENDING','ACCEPTED','IN_PROGRESS') ORDER BY created_at DESC LIMIT 1`;
      ...
      if (action === 'accept') {
        ...
        await audit(u, { action: 'assignment.accepted', entityType: 'complaint', entityId: code });
      } else if (action === 'start') {
        requireStatus('ASSIGNED');
        await sql`UPDATE assignments SET status = 'IN_PROGRESS', started_at = now(), accepted_at = COALESCE(accepted_at, now()) WHERE id = ${a.id}`;
        ...
        await audit(u, { action: 'complaint.progress', entityType: 'complaint', entityId: code, newValue: { pct, notes } });
      } else {
        requireStatus('IN_PROGRESS');
        const notes = z.string().trim().min(3).max(2000).parse(data.notes);
        ...
        await sql`UPDATE assignments SET status = 'COMPLETED', completed_at = now() WHERE id = ${a.id}`;
    ...
    case 'escalate': {
      ...
      await audit(u, { action: 'complaint.escalated', entityType: 'complaint', entityId: code, oldValue: { escalated: c.escalated }, newValue: { escalated: true, note } });
      const eos = await sql`SELECT u.id FROM users u JOIN roles r ON r.id = u.role_id JOIN officials o ON o.user_id = u.id
                            WHERE r.code = 'EO' AND u.status = 'ACTIVE' AND o.local_body_id = ${c.local_body_id}`;
    ...
    case 'remark': {
      ...
      await audit(u, { action: 'complaint.remark', entityType: 'complaint', entityId: code, newValue: { note } });
      break;
    }
    ...
    case 'appeal_decide': {
      ...
      await audit(u, { action: `appeal.${decision.toLowerCase()}`, entityType: 'complaint', entityId: code, newValue: { appealId, notes } });
...
async function assignableUser(u: AuthUser, userId: string, allowSelf: boolean) {
  ...
  const [row] = await sql`
    ...
    WHERE usr.id = ${userId} AND ${assignableScope(u)}`;
